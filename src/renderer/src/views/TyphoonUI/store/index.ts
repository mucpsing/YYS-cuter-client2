...
export const useTyphoonFileStore = defineStore("fileStore", {
  state: () => ({
    currtFileMd5: "", // 当前选择的文件
  ...
  getters: {
    ...
    fileMd5Options: (state) =>
      Object.values(state.fileObj).map((item) => ({ label: item.filename, value: item.md5 })),
  },
  ...
  actions: {
    ...
    async addDataFromTxt(file: File) {
      ...
      this.fileObj[res.md5] = {
        md5: await getMd5(file),
        filename: file.name,
        file,
        parserData: resList,
      }
