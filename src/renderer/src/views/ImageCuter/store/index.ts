 ...
 * @LastEditTime: 2023-08-10 11:02:46
...
export const localStore = reactive({
  ...
  addCoords(mode: "add" | "update" | "auto" = "auto"): boolean {
    ...
    if (mode == "auto") {
      sameTitle =
        this.crop.title == this.currtCoordsList[this.currtCoordsIndex].title;
      hasTitle = Boolean(
        this.currtCoordsList.find((item) => item.title == this.crop.title)
      );
    ...
    switch (mode) {
      case "add":
        ...
        this.currtCoordsIndex =
          this.currtCoordsList.length > 0 ? this.currtCoordsList.length - 1 : 0;
        return true;

      case "update":
        // 当前选中的修改坐标
        this.currtCoordsList[this.currtCoordsIndex].title =
          this.crop.title.toString();
        this.currtCoordsList[this.currtCoordsIndex].startX = Math.trunc(
          this.crop.startX
        );
        this.currtCoordsList[this.currtCoordsIndex].startY = Math.trunc(
          this.crop.startY
        );
        this.currtCoordsList[this.currtCoordsIndex].endX = Math.trunc(
          this.crop.endX
        );
        this.currtCoordsList[this.currtCoordsIndex].endY = Math.trunc(
          this.crop.endY
        );
  ...
  async switchCoords(index: number) {
    ...
    this.crop.title =
      this.currtCoordsList[this.currtCoordsIndex].title.toString();
    this.crop.startX = Math.trunc(
      this.currtCoordsList[this.currtCoordsIndex].startX
    );
    this.crop.startY = Math.trunc(
      this.currtCoordsList[this.currtCoordsIndex].startY
    );
    this.crop.endX = Math.trunc(
      this.currtCoordsList[this.currtCoordsIndex].endX
    );
    this.crop.endY = Math.trunc(
      this.currtCoordsList[this.currtCoordsIndex].endY
    );
